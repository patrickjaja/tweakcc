...
export function saveThemesToConfig(themes: Theme[]): void {
  const config = getConfig() || { ccVersion: '', lastModified: new Date().toISOString() };
...
export function saveLaunchTextToConfig(launchTextConfig: LaunchTextConfig): void {
  const config = getConfig() || { ccVersion: '', lastModified: new Date().toISOString() };
  if (!config.settings) config.settings = {};
...
export function saveThinkingVerbsToConfig(thinkingVerbsConfig: ThinkingVerbsConfig): void {
  const config = getConfig() || { ccVersion: '', lastModified: new Date().toISOString() };
  if (!config.settings) config.settings = {};
...
export function saveThinkingStyleToConfig(thinkingStyleConfig: ThinkingStyleConfig): void {
  const config = getConfig() || { ccVersion: '', lastModified: new Date().toISOString() };
  if (!config.settings) config.settings = {};
