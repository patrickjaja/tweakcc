...
export default function App() {
  ...
  useEffect(() => {
    const performStartupCheck = async () => {
      ...
      if (result.needsUpdate) {
        setUpdateNotification('Claude Code version update available. Applying update...');
      ...
      if (savedThinkingVerbs) {
        setState(prev => ({ ...prev, thinkingVerbsConfig: savedThinkingVerbs }));
      }
      
      const savedThinkingStyle = getThinkingStyleFromConfig();
      if (savedThinkingStyle) {
        setState(prev => ({ ...prev, thinkingStyleConfig: savedThinkingStyle }));
      }
    };
  ...
  const handleRestoreOriginalClaude = async () => {
    try {
      ...
      if (success) {
        ...
        setUpdateNotification('Failed to restore - no backup found or restore error.');
        setTimeout(() => setUpdateNotification(null), 5000);
      }
    } catch (error) {
  ...
  const handleOpenTweakccConfig = () => {
    try {
      ...
      if (!existsSync(configPath)) {
        setUpdateNotification('Config file not found. Creating default config...');
      ...
      setUpdateNotification('Opening file browser with tweakcc.json selected...');
      setTimeout(() => setUpdateNotification(null), 2000);
    } catch (error) {
      setUpdateNotification(`Failed to open tweakcc.json: ${error instanceof Error ? error.message : 'Unknown error'}`);
      setTimeout(() => setUpdateNotification(null), 3000);
    }
  };
  ...
  const handleApplyChanges = () => {
    ...
    try {
      ...
      if (savedLaunchText) {
        ...
        } else if (savedLaunchText.method === 'figlet' && savedLaunchText.figletText) {
          try {
            textToApply = figlet.textSync(savedLaunchText.figletText.replace('\n', ' '), {
      ...
      const savedThinkingVerbs = getThinkingVerbsFromConfig();
      if (savedThinkingVerbs) {
        const haikuResult = writeUseHaikuVerbs(content, savedThinkingVerbs.useHaikuGenerated);
        if (haikuResult) {
          content = haikuResult;
        } else {
        ...
        const punctResult = writeThinkerPunctuation(content, savedThinkingVerbs.punctuation);
        if (punctResult) {
          content = punctResult;
        } else {
      ...
      const savedThinkingStyle = getThinkingStyleFromConfig();
      if (savedThinkingStyle) {
        const charsResult = writeThinkerSymbolChars(content, savedThinkingStyle.phases);
        if (charsResult) {
          content = charsResult;
        } else {
        ...
        const speedResult = writeThinkerSymbolSpeed(content, savedThinkingStyle.updateInterval);
        if (speedResult) {
          content = speedResult;
        } else {
        ...
        const mirrorResult = writeThinkerSymbolMirrorOption(content, savedThinkingStyle.reverseMirror);
        if (mirrorResult) {
      ...
      throw error;
      setUpdateNotification(`Failed to apply changes: ${error instanceof Error ? error.message : 'Unknown error'}`);
      setTimeout(() => setUpdateNotification(null), 5000);
    }
  };
  ...
  const handleOpenCliJs = () => {
    try {
      ...
      setUpdateNotification(`Failed to open cli.js: ${error instanceof Error ? error.message : 'Unknown error'}`);
      setTimeout(() => setUpdateNotification(null), 3000);
    }
  };
